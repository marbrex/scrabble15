...
public class GameFinderController implements LobbyController {
  /**
   * Controller of the GameFinder.fxml screen, which have the function to search specific lobbys in
   * the network and gave the player the option to join them if possible.
   * 
   * @author hendiehl
   */
  @FXML
  ...
  /**
   * Method to deactivate the control fields for a specific port given by the user and inform the
   * player how he can activate the normal procedure again.
   * 
   * @author hendiehl
   */
  ...
  /**
   * Method to handle the ActionEvent of the joinBtn in reason to join a founded lobby in the
   * network.
   * 
   * @author hendiehl
   */
  ...
  /**
   * Method to check if an String matches the characteristics of an port number, which means that it
   * have to be a number of a minimum length of 4.
   * 
  ...
  /**
   * Method which handles the actionEvent of the backButton and calls the openScreen method with the
   * Menu.fxml and shut down a running client protocol.
   * 
   * @author hendiehl
   */
  @FXML
  ...
  public void openMenu() {
    Platform.runLater(() -> {
      try {
         ...
         * Scene(root, this.root.getScene().getWidth(), this.root.getScene().getWidth()));
         */
  ...
  /**
   * Method which will be called after a lobby is found and connected successfully, the join button
  ...
  /**
   * Method which will be called when no lobby is founded, the player will be informed that no lobby
  ...
  /**
   * Method which handles the useOwnPort ActionEvents and activates or deactivates the controls for
   * an user input.
   * 
   * @author hendiehl
   */
  ...
  /**
   * Method which handles the searchButton ActionEvent and try to create an instances of an client
   * protocol with an specific port given by the user. The input is controlled and checked. A valid
   * port has to be a port number in range or the 0 for standard port. The player is informed about
   * the success.
   * 
   * @author hendiehl
   */
  ...
  /**
   * Method to navigate from the GameFinder screen to the GameLobby screen as a client, which sets
   * also the client protocol for the lobby controller.
   * 
   * @author hendiehl
   */
  public void goInLobby() {
    Platform.runLater(() -> {
      try {/*
        ...
        /*
         * stage.setScene(new Scene(root, this.root.getScene().getWidth(),
         * this.root.getScene().getHeight())); stage.setMinWidth(900); stage.setMinHeight(800);
